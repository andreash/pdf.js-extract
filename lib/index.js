...
class PDFExtract {
	...
	extractBuffer(buffer, options = {}, cb) {
		...
		pdfjsLib.getDocument(options).promise.then(doc => {
			...
			const getOutlinePageNums = (outline) => {
				return Promise.all(
					outline.map(item => {
						let promise = Promise.resolve();
						if (item.items.length > 0) {
							promise = getOutlinePageNums(item.items);
						}
						return promise.then(()=>doc.getPageIndex(item.dest[0]).then(pgIdx => {
							item.pageIndex = pgIdx;
						}))
					}))
			}
			...
			const promises = [
				...
				doc.getOutline().then(outline => {
					pdf.outline = outline;
					return getOutlinePageNums(outline);
					doc.getPageIndex(outline[0].dest[0]).then(pgIdx => {
						console.log(pgIdx);
					});
				})
			];
